...
// TODO: Auto upgrade&err detection & disconnect

var version = "`PROJECT AKARIN-CENTRAL VERSION 20160416`";
...
var connection = jf.readFileSync("connections.json");
var connection_i2t = {};
...
function format_newline(msg, user, target, type) {
    var text = msg.text.replace(/(\s*\n\s*)+/g, '\n');
    if(type == 'reply')
        text = text.replace(/\n/g, printf('\n[%1] %2: ', user, target));
    if(type == 'forward')
        text = text.replace(/\n/g, printf('\n[%1] Fwd %2: ', user, target));
    ...
    if (arr.length > config.irc_line_count_limit ||
        ...
        if(config.irc_long_message_paste_enabled){
            ...
            pvimcn.pvim(text, function cb(err, result){
                ...
                if(err)
                    irc_c.say(connection[msg.chat.id],
                               printf('[%1] %2', user,
                                      text.replace(/\n/g, '\\n')));
                else
                    irc_c.say(connection[msg.chat.id],
...
irc_c.addListener('message', function (from, to, message) {
...
function sendimg(fileid, msg, type){
    ...
    tg.getFileLink(fileid).then(function (ret){
        ...
        pvimcn.imgvim(url, function(err,ret){
            ...
            if (msg.caption){
                irc_c.say(connection[msg.chat.id], printf("[%1] %2: %3 Saying: %4", user, type, ret.trim(), msg.caption));
            }else{
                irc_c.say(connection[msg.chat.id], printf("[%1] %2: %3", user, type, ret.trim()));
            }
        });
    });
}
...
tg.on('message', function(msg) {
    ...
    if(config.irc_photo_forwarding_enabled && msg.photo && connection[msg.chat.id]){
    ...
    } else if (config.irc_photo_forwarding_enabled && msg.sticker && connection[msg.chat.id]){
        sendimg(msg.sticker.file_id, msg, 'Sticker');
    } else if (config.irc_photo_forwarding_enabled && msg.voice && connection[msg.chat.id]){
        sendimg(msg.voice.file_id, msg, 'Voice');
    } else if (config.irc_photo_forwarding_enabled && msg.document && connection[msg.chat.id]){
        ...
        if (connection[msg.chat.id]) {
            ...
            /*} else if (command[0] == '/blocki2t' || command[0] == '/blocki2t@' + tgusername) {
                ...
                command.shift();
                txtn = command.join(" ");
                irc_c.say(connection[msg.chat.id], txtn);
                return;
            ...
            /*} else if (command[0] == '/blocklist' || command[0] == '/blocklist@' + tgusername) {
                ...
                if (command[1]) {
                    ...
                    irc_c.say(connection[msg.chat.id], notifymsg);
                } else {
                    var first_name = msg.from.first_name;
                    var last_name = msg.from.last_name;
                    var full_name = last_name?
    ...
    // Message Filter
    if(!msg.text || msg.chat.id >= 0 || disabled[connection[msg.chat.id]] || msg.date < inittime)
        return;
        /*
    // Blocking Enforcer
    ...
    if(msg.reply_to_message){
        ...
        message_text = format_newline(msg, user, reply_to, 'reply');
        ...
        message_text = format_newline(msg, user, forward_from,
                                      'forward', true);
        if(message_text === null) return;
        message_text = printf('[%1] Fwd %2: %3', user, forward_from, message_text);
    } else {
        message_text = format_newline(msg, user);
        if(message_text === null) return;
        message_text = printf('[%1] %2', user, message_text);
    }
    if(me_message){
        irc_c.action(connection[msg.chat.id], message_text);
    }else{
        irc_c.say(connection[msg.chat.id], message_text);
    }
...
function resetTg(){
    //tg.sendMessage(config.tg_group_id, "`REQUESTED RESET BY USER`", { parse_mode: 'Markdown' }); // May not sent.
    for (i in connection_i2t) {
        irc_c.part(i);
    }
    irc_c.disconnect();
...
var cutinv = config.cutjj_interval ? config.cutjj_interval : 60 * 1000;
